...
- `/dev-planning`でPRD・アーキテクチャ・Storyファイルの下準備が完了していることが望ましいが、未実施の場合は手順1で`code_search`/`read_file`を用いて必要情報を自力収集する。
...
4. Story更新とハンドオーバー
   ...
   - 次ステップとして実行すべきワークフロー（例: `/dev-swarm-qa`、リリース準備）を明記する。後続ワークフローが無い場合は手動対応のチェックリストと責任者を記載する。

...
本リポジトリのワークフローは、npm 経由で簡単にインストールできます。

```bash
npx yodogawa
```

実行すると、**Windsurf (.windsurf)**、**Antigravity (.agent)**、または **Cursor (.cursor)** を選択でき、自動的に必要なファイルがプロジェクトにコピーされます。

Cursor を選択した場合、ワークフローファイルは `.cursor/commands` に配置され、Cursor のコマンドとして利用できるようになります。
